...
async def mail_router(mail_text: str) -> dict:
    ...
    async with FastMCPClient(API_URL, timeout=10000) as client:
        ...
        if tools:
            ...
            # Check if tool_name is "analyze_credit" and result contains "status": "approved"
            if tool_name == "analyze_credit" and isinstance(result, dict) and result.get("status") == "approved":
